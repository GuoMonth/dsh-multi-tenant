...
每次验证固定双仓库提交、DSH 基线和镜像身份。固定版本是可复现实验边界，不是长期版本冻结。对外发行必须锁定可公开拉取的 Cell/Operator 镜像 digest，并明确对应的 DSH 精确版本及源码提交；部署的平台镜像同样使用 digest。npm latest 是安装通道，不是运行时镜像漂移或版本范围兼容承诺。新迭代可以更换组合，但必须明确记录并验证，不能改写已发布制品身份。
