...
Status: **release-ready source, intentionally unpublished**. The focused implementation and adversarial closure review completed on 2026-09-03. npm publication, a Git tag, and a GitHub Release remain separate explicit actions and were not performed.
...
状态：**源码已达到 release-ready，按计划保持未发布**。`0.4.0-alpha.2` 没有改变项目定位，仍然只负责 `PrincipalContext -> AgentId -> Principal-scoped Directory -> DSH Agent/Session/MCP`。npm、Git tag 和 GitHub Release 均未创建。
