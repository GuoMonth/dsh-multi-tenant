...
用户可在自己的可写目录执行代码、启动子进程、安装用户态依赖并修改文件；任务可能破坏自己的工作环境，这不是平台替用户恢复的责任。并不因此开放 privileged、宿主机挂载、Docker socket、hostNetwork 或集群凭据。默认非 root 与 RuntimeDefault seccomp 成本很低，保留；系统级软件放进固定镜像。若真实 POC 任务需要更宽的容器文件写入范围，再单独调整挂载或镜像，不能把“移除额外 sandbox”等同于关闭所有容器限制。
...
必须保留：OIDC 与跨 owner 校验、平台凭据不进用户 Pod、独立用户卷、无宿主权限、CPU/内存与临时空间限制、阻断跨租户和控制面/私有地址的非必要流量，模型 API 等必要外网访问按参考环境配置。网络策略要求 CNI 实际执行；Pod 默认存在不意味着网络已经隔离。相信 AI 的指令遵循是使用预期，不是访问控制措施。
...
| P3 一次闭环验收 | 固定两用户、一个部署组合；补充真实安装记录，修完当前链路问题后才发下一 Alpha | 登录→自动分配/进入自己的 Cell→真实模型完成一项文件/命令任务→刷新可继续；B 不能访问 A；登出关闭访问；普通 Pod 重建文件保留 |
