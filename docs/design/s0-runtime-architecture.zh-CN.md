...
内部端口归 runtime 维护；平台通过组合根接入，业务层不导入 K8s 类型或判断 backend。当前一个实现足够，不先拆多个发行包、另起 runtime 服务或冻结 JSON/HTTP 线协议。下一版明确删除 Process/Docker 产品后端，不再预留多后端兼容；见新版 AgentEnvironment 契约。
