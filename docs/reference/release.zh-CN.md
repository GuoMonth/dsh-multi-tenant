...
Release identity 是 `dsh-multi-tenant@0.5.0`，对应 Git tag 为 `v0.5.0`。npm 分发使用 `latest` dist-tag。唯一支持的 Harness 基线是 DSH `0.1.5-alpha.1`，对应 commit `5dda764ed3aa172535a7967b06ff95d9cbfe536a`。
...
CI 在 Node 22.19 和 Node 24 上重复执行，并单独 checkout DSH commit `5dda764ed3aa172535a7967b06ff95d9cbfe536a`，核对精确的 `0.1.5-alpha.1` 源码身份。
...
1. 将发布 PR 合入 `main`，确认已提交的 package version 为 `0.5.0`。
...
4. 核对 `dsh-multi-tenant@0.5.0`、npm `latest`、Git tag `v0.5.0` 和对应 GitHub Release。npm 版本不可覆盖，修正须使用新版本。
