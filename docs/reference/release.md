...
The release identity is `dsh-multi-tenant@0.6.0` with matching Git tag `v0.6.0`. npm distribution uses the `latest` dist-tag. Its only supported Harness baseline is DSH `0.1.5-rc.2` at commit `fb2c4b9e698e30edb738bca4cf0618587db7d203`.
...
CI repeats the checks on Node 22.19 and Node 24 and separately checks out DSH commit `fb2c4b9e698e30edb738bca4cf0618587db7d203` to verify the exact `0.1.5-rc.2` source identity.
...
1. Merge the release PR into `main`; verify the committed package version is `0.6.0`.
...
4. Verify `dsh-multi-tenant@0.6.0`, npm `latest`, Git tag `v0.6.0`, and the matching GitHub Release. npm versions are immutable; a correction requires a new version.
