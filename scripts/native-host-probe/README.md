...
The user-approved authority boundary is `(tenantId, principalId)`. The root publication counterexample remains as documentation of native semantics, not a WP5 blocker: independent root read grants are not part of this product. See [the current plan](../../docs/archive/native-domain-refactor-plan.zh-CN.md) and [WP5 evidence](../../docs/evidence/native-domain-review/wp5/README.md). Earlier probe variants and reports above are retained as historical/reference evidence.
